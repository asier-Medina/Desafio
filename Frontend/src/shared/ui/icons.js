export {
  FaRegHeart,                // favorito (sin marcar)
  FaHeart,                   // favorito (activo)
  FaRegUser,                 // usuario / iniciar sesión
  FaSliders,                 // filtros
  FaChevronDown,             // desplegables (menú usuario)
  FaLocationDot,             // ubicación / mapa
  FaRegCalendarPlus,         // añadir a calendario
  FaRegCalendar,             // eventos
  FaUtensils,                // gastronomía
  FaLandmark,                // cultura
  FaMagnifyingGlass,         // búsqueda
  FaArrowLeft,               // volver atrás
  FaArrowUpRightFromSquare,  // enlace externo (comprar entradas / ir a la web)
  FaStar,                    // valoración / estrella
  FaArrowRight,              // flecha derecha
  FaBars,                    // menú hamburguesa
  FaXmark,                   // cerrar / equis
  FaRegClock,                // reloj / hora
  FaEuroSign,                // precio / euro
  FaLink,                    // enlace
} from "react-icons/fa6";