    ...
    <div className="profile">
        ...
        <div className="profile__card">
          ...
          {editing ? (
            ...
            <dl className="profile__info-list">
              ...
              {profile.municipality_id && <div className="profile__info-row"><dt>Municipio</dt><dd>{getNameById(user.municipality_id)}</dd></div>}
              {user.sexo && <div className="profile__info-row"><dt>Sexo</dt><dd>{SEXO_OPTS.find(o => o.value === profile.sexo)?.label ?? profile.sexo}</dd></div>}
              {profile.age && <div className="profile__info-row"><dt>Edad</dt><dd>{profile.age} años</dd></div>}
              {user.createdAt && <div className="profile__info-row"><dt>Miembro desde</dt><dd>{new Date(user.createdAt).toLocaleDateString("es", { year: "numeric", month: "long" })}</dd></div>}
            </dl>
          )}
        </div>
      )}
      ...
      {user && (
        <div className="profile__card">
          ...
          {editing ? (
            loading ? <p className="profile__no-data">Cargando…</p>
            : catalog.length === 0 ? <p className="profile__no-data">No se pudo cargar el catálogo de intereses.</p>
            : (
      ...
      {user && (
        <div className="profile__card">
          ...
          : hasPrefs ? (
            <dl className="profile__info-list">
              {prefs.rango_precio && <div className="profile__info-row"><dt>Precio</dt><dd>{PRECIO_OPTS.find(o => o.value === prefs.rango_precio)?.label}</dd></div>}
              {prefs.movilidad_reducida && <div className="profile__info-row"><dt>Accesibilidad</dt><dd>Movilidad reducida</dd></div>}
              {selectedMuniNames.length > 0 && <div className="profile__info-row"><dt>Municipios</dt><dd>{selectedMuniNames.join(", ")}</dd></div>}
            </dl>
          ) : (
            <p className="profile__no-data">
      ...
      <section className="profile__section">
        ...
        {favorites.length === 0 ? (
          ...
          <div className="profile__favorites">
            {favorites.map(item => (
              <Card key={`${item._variant}-${item.id}`} variant={item._variant} data={item}
                isFavorite={true}
